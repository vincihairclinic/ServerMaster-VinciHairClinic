...
<div class="form-group {{ !empty($selectedValue) || (!empty($model) && !empty($model->{$id})) ? 'is-dirty' : '' }} {{ !empty($cell) ? 'col-xl-'.$cell[0].' col-md-'.$cell[1].' col-'.$cell[2] : ''}} {{ !empty($cssClass) ? $cssClass : '' }}" style="{{ !empty($style) ? $style : '' }}">
    <label>{{ $title }}</label>
    <select class="chosen-select form-control" name="{{ isset($name) ? $name : $id }}" id="{{ !empty($id_) ? $id_ : (isset($name) ? $name : $id)  }}" {{ isset($inputAttr) ? $inputAttr : '' }} {!! isset($onchange) ? 'onchange="'.$onchange.'"' : '' !!}>
