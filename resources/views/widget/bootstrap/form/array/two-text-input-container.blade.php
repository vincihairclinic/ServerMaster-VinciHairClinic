<div class="form-group row mx-0 col-12" id="add-new-text-input-{{ $id }}-container">
    @foreach( old($id, $model->{$id}) as $i => $item)
        <div class="col-12 row p-0 m-0 position-relative">
            @include('widget.bootstrap.form.array.text-input', ['arrayName' => $id, 'name' => $id, 'property' => $firstProperty ?? 'input', 'title' => $firstTitle, 'value' => ((array)$item)[$firstProperty ?? 'input'] ?? null, 'cell' => [6, 6, 12], 'close' => false])
            @include('widget.bootstrap.form.array.text-input', ['arrayName' => $id, 'name' => $id, 'property' => $secondProperty ?? 'input', 'title' => $secondTitle, 'value' => ((array)$item)[$secondProperty ?? 'input'] ?? null, 'cell' => [6, 6, 12], 'close' => false])
            @if($close ?? true)
...
<template id="add-new-text-input-{{$id}}-template">
    <div class="col-12 row p-0 m-0 position-relative">
        @include('widget.bootstrap.form.array.text-input', ['arrayName' => $id, 'name' => $id, 'property' => $firstProperty ?? 'input', 'title' => $firstTitle, 'i' => '${i}', 'cell' => [6, 6, 12], 'close' => false])
        @include('widget.bootstrap.form.array.text-input', ['arrayName' => $id, 'name' => $id, 'property' => $secondProperty ?? 'input', 'title' => $secondTitle, 'i' => '${i}', 'cell' => [6, 6, 12], 'close' => false])
