...
@section('head_title') {{ $model->id ? 'Edit ID:'.$model->id : 'Add article category' }} @endsection
@section('title')
    <a href="{{ route('dashboard.article-category.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
    {{ $model->id ? 'Edit ID:'.$model->id : 'Add article category' }}
@endsection
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.article-category.update' : 'dashboard.article-category.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
            ...
            <div class="col-12 footer">
                @include('widget.bootstrap.form.action-buttons', [
                    'cancelUrl' => route('dashboard.article-category.index'),
                ])
        ...
        @if($model->id)
            <form id="destroy-form" action="{{ route('dashboard.article-category.destroy', ['model' => $model->id]) }}" method="POST" style="display: none;"> @csrf </form>
        @endif
    </div>
@endsection
