...
@section('head_title') {{ $model->id ? 'Edit ID:'.$model->id : 'Add hair type' }} @endsection
@section('title')
    <a href="{{ route('dashboard.hair-type.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
    {{ $model->id ? 'Edit ID:'.$model->id : 'Add hair type' }}
@endsection
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.hair-type.update' : 'dashboard.hair-type.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
            ...
            <div class="col-12 footer">
                @include('widget.bootstrap.form.action-buttons', [
                    'cancelUrl' => route('dashboard.hair-type.index'),
                    'hideDelete' => true,
                ])
