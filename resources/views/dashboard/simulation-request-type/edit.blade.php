...
@section('head_title') {{ $model->id ? 'Edit ID:'.$model->id : 'Add simulation request type' }} @endsection
@section('title')
    <a href="{{ route('dashboard.simulation-request-type.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
    {{ $model->id ? 'Edit ID:'.$model->id : 'Add simulation request type' }}
@endsection
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.simulation-request-type.update' : 'dashboard.simulation-request-type.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
            ...
            @include('widget.bootstrap.form.wide-card-image', ['id' => 'image', 'cell' => [3, 4, 12],
            ...
            <div class="col-12 footer">
                @include('widget.bootstrap.form.action-buttons', [
                    'cancelUrl' => route('dashboard.simulation-request-type.index'),
                ])
        ...
        @if($model->id)
            <form id="destroy-form" action="{{ route('dashboard.simulation-request-type.destroy', ['model' => $model->id]) }}" method="POST" style="display: none;"> @csrf </form>
        @endif
    </div>
@endsection
