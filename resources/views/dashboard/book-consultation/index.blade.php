...
@section('head_title') User book consultation @endsection
@section('title') User book consultation @endsection
    ...
    <script type="text/javascript">
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.book-consultation.index-json') }}',
                    type: 'post'
                },
                order: [
                    [0, "desc"],
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return `<a class="btn btn-self-info py-1 px-0 text-center width-60" href="{{ route('dashboard.book-consultation.index') }}/${row.id}/edit">Check</a>
            ...
            {{--    if (data[0] !== undefined) {--}}
            {{--        $.ajax({--}}
            {{--            url: '{{ route('dashboard.book-consultation.sort-update') }}',--}}
            {{--            method: 'post',--}}
            {{--            data: {--}}
            {{--                data: data--}}
            {{--            }--}}
        ...
        function updateSalesForce(id) {
            $.ajax({
                url: `{{ route('dashboard.book-consultation.index') }}/${id}/update`,
                method: 'POST',
                data: {
                    ajax : true,
                },
