...
@section('head_title') User simulation request @endsection
@section('title') User simulation request @endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('dashboard.user-simulation-request.create') }}" class="btn btn-self-primary">Add user simulation request</a>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.user-simulation-request.index-json') }}',
                    type: 'post'
                },
                order: [
                    [0, "desc"],
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return '<button type="button" class="p-0 border-0" data-toggle="modal" data-target="#dialog-edit-image" data-image="image_'+row.id+'"> ' +
                                '<img onerror="this.src=\''+defaultImageUrl+'\'" class="width-50 rounded border'+(row.hair_front_image ? ' cursor-pointer' : '')+'" id="image_'+row.id+'" src="'+(row.url_hair_front_image ? row.url_hair_front_image + '?r='+Math.random() : defaultImageUrl)+'" onclick="showDialogEditImage(this, true)">' +
                                '</button>';
                        }
                    },{
                        data: 'email',
                        ...
                        render: function (data, type, row) {
                            return '<a class="btn btn-self-info py-1 px-0 text-center width-60" href="{{ route('dashboard.user-simulation-request.index') }}/' + row.id + '/edit">Edit</button>' +
                                ' <a class="btn btn-self-danger py-1 ml-2 px-0 text-center width-60" href="javascript:void(0);" onclick="removeDataTableRow(\'{{ route('dashboard.user-simulation-request.index') }}/' + row.id + '/destroy\')">Delete</a>';
                        }
                    }
                ],
            ...
            {{--    if (data[0] !== undefined) {--}}
            {{--        $.ajax({--}}
            {{--            url: '{{ route('dashboard.user-simulation-request.sort-update') }}',--}}
            {{--            method: 'post',--}}
            {{--            data: {--}}
            {{--                data: data--}}
            {{--            }--}}
