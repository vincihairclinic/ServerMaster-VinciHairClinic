...
@section('head_title') {{ $model->id ? 'Edit ID:'.$model->id : 'Add hair loss type' }} @endsection
@section('title')
    <a href="{{ route('dashboard.hair-loss-type.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
    {{ $model->id ? 'Edit ID:'.$model->id : 'Add hair-loss-type' }}
@endsection
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.hair-loss-type.update' : 'dashboard.hair-loss-type.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
            ...
            @include('widget.bootstrap.form.chosen-select-single', ['id' => 'gender_id', 'title' => 'Gender', 'cell' => 6, 'inputAttr' => 'autocomplete = off required',
                'options' => \App\Models\Gender::all(),
            ])

            <div class="col-12 footer">
                @include('widget.bootstrap.form.action-buttons', [
                    'cancelUrl' => route('dashboard.hair-loss-type.index'),
                    'hideDelete' => true,
                ])
