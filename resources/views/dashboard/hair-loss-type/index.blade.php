...
@section('head_title') Hair loss types @endsection
@section('title') Hair loss types @endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('dashboard.hair-loss-type.create') }}" class="btn btn-self-primary">Add hair loss type</a>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.hair-loss-type.index-json') }}',
                    type: 'post'
                },
                order: [
                    [0, "desc"],
                ...
                columns: [
                    {
                        ...
                        searchable: false,
                        orderable: false,
                        className: 'text-truncate center width-150 py-2 align-middle sort-row-target',
                        render: function (data, type, row) {
                            return '<button type="button" class="p-0 border-0" data-toggle="modal" data-target="#dialog-edit-image" data-image="image_'+row.id+'"> ' +
                                '<img onerror="this.src=\''+defaultImageUrl+'\'" class="width-120 rounded'+(row.image ? ' cursor-pointer' : '')+'" id="image_'+row.id+'" src="'+(row.url_image ? row.url_image + '?r='+Math.random() : defaultImageUrl)+'" onclick="showDialogEditImage(this, true)">' +
                                '</button>';
                        }
                    },{
                        data: 'name_en',
                        ...
                        render: function (data, type, row) {
                            return '<a class="btn btn-self-info py-1 px-0 text-center width-60" href="{{ route('dashboard.hair-loss-type.index') }}/' + row.id + '/edit">Edit</button>' +
                                ' <a class="btn btn-self-danger py-1 ml-2 px-0 text-center width-60" href="javascript:void(0);" onclick="removeDataTableRow(\'{{ route('dashboard.hair-loss-type.index') }}/' + row.id + '/destroy\')">Delete</a>';
                        }
                    }
                ],
            ...
            dataTable.on('row-reorder', function (e, diff, edit) {
                ...
                if (data[0] !== undefined) {
                    $.ajax({
                        url: '{{ route('dashboard.hair-loss-type.sort-update') }}',
                        method: 'post',
                        data: {
                            data: data
                        }
