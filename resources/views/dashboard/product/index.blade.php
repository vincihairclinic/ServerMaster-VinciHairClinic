...
@section('head_title') Products @endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('dashboard.product.create') }}" class="btn btn-self-primary">Add product</a>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.product.index-json') }}',
                    type: 'post'
                },
                order: [
                    [0, "desc"],
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return '<button type="button" class="p-0 border-0" data-toggle="modal" data-target="#dialog-edit-image" data-image="image_'+row.id+'"> ' +
                                '<img onerror="this.src=\''+defaultImageUrl+'\'" class="width-50 rounded'+(row.images ? ' cursor-pointer' : '')+'" id="image_'+row.id+'" src="'+(row.url_images[0] ? row.url_images[0] + '?r='+Math.random() : defaultImageUrl)+'" onclick="showDialogEditImage(this, true)">' +
                                '</button>';
                        }
                    },{
                        data: 'name_en',
                        ...
                        render: function (data, type, row) {
                            return '<a class="btn btn-self-info py-1 px-0 text-center width-60" href="{{ route('dashboard.product.index') }}/' + row.id + '/edit">Edit</button>' +
                                ' <a class="btn btn-self-danger py-1 ml-2 px-0 text-center width-60" href="javascript:void(0);" onclick="removeDataTableRow(\'{{ route('dashboard.product.index') }}/' + row.id + '/destroy\')">Delete</a>';
                        }
                    }
                ],
            ...
            dataTable.on('row-reorder', function (e, diff, edit) {
                ...
                if (data[0] !== undefined) {
                    $.ajax({
                        url: '{{ route('dashboard.product.sort-update') }}',
                        method: 'post',
                        data: {
                            data: data
                        }
