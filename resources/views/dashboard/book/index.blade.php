...
@section('head_title') Books @endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('dashboard.book.create') }}" class="btn btn-self-primary">Add book</a>
        </div>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.book.index-json') }}',
                    type: 'post'
                },
                order: [
                    [0, "desc"],
                ...
                columns: [
                    {
                        ...
                        render: function (data, type, row) {
                            return '<a class="btn btn-self-info py-1 px-0 text-center width-60" href="{{ route('dashboard.book.index') }}/' + row.id + '/edit">Edit</button>' +
                                ' <a class="btn btn-self-danger py-1 ml-2 px-0 text-center width-60" href="javascript:void(0);" onclick="removeDataTableRow(\'{{ route('dashboard.book.index') }}/' + row.id + '/destroy\')">Delete</a>';
                        }
                    }
                ],
            ...
            dataTable.on('row-reorder', function (e, diff, edit) {
                ...
                if (data[0] !== undefined) {
                    $.ajax({
                        url: '{{ route('dashboard.book.sort-update') }}',
                        method: 'post',
                        data: {
                            data: data
                        }
