...
@section('title')
    <a href="{{ route('dashboard.request-contact-from-clinic.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.request-contact-from-clinic.update' : 'dashboard.request-contact-from-clinic.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
            ...
            <div class="col-12 footer">
                @include('widget.bootstrap.form.action-buttons', [
                    'cancelUrl' => route('dashboard.request-contact-from-clinic.index'),
                    'saveBtn' => false,
                    'hideDelete' => true,
                ])
