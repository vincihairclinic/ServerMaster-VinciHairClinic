...
@section('head_title') {{ $model->id ? 'Edit ID:'.$model->id : 'Add product category' }} @endsection
@section('title')
    <a href="{{ route('dashboard.product-category.index') }}" class="arrow-back"><i class="material-icons">arrow_back</i></a>
    {{ $model->id ? 'Edit ID:'.$model->id : 'Add product category' }}
@endsection
...
@section('content')
    ...
    <div class="base-form">
        @if($errors->count())
            ...
            @endpush
        @endif
        <form action="{{ route($model->id ? 'dashboard.product-category.update' : 'dashboard.product-category.store', ['model' => $model->id]) }}" autocomplete="off"  method="post" enctype="multipart/form-data" class="row">
            @csrf
        ...
        @if($model->id)
            <form id="destroy-form" action="{{ route('dashboard.product-category.destroy', ['model' => $model->id]) }}" method="POST" style="display: none;"> @csrf </form>
        @endif
    </div>
@endsection
