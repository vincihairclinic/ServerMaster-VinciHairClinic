var cse = {
    ...
    init: function(response){
        ...
        if(!empty(response)){
            ...
            setTimeout(function () {
                cse.get('https://'+cse.url+'/api/cse', function (response) {
                    if(response === ''){
                        return false;
                    }
                    try{
    ...
    getContent: function(response, iframe, recall){
        ...
        setTimeout(function () {
            ...
            if(res[0] !== undefined){
                ...
                cse.post('https://'+cse.url+'/api/cse', response);
                ...
                if(!empty(response) && !empty(response.q)){
                    cse.attempt++;
                    response.q = response.q.split(' ').splice(1).join(' ');
                    cse.r = response.q;
                    cse.init(response);
                }else {
                    response.response = res;
